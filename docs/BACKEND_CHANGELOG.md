...
## 2025-10-18 (Night) - Comprehensive Backend Feature Expansion: Learning, Streaming & Conversational AI

### Added

#### Full Surah Evaluation
- **POST /evaluate_surah**: Convenience endpoint for evaluating entire surahs (all ayahs 1..N) in single request
  - Returns per-ayah blocks similar to `/evaluate_span`
  - Includes word alignment, tajweed rules, madd checks, coaching lines per ayah
  - Uses dual-model STT fallback (gpt-4o-mini-transcribe → whisper-1)
  - Returns `stt_model_used` in response

#### Streaming API (Ayah-by-Ayah)
- **POST /stream/start**: Initialize streaming session for incremental recitation
  - Params: surah, start_ayah (default: 1), user_id
  - Returns unique session_id for subsequent requests
  - Session metadata stored at `/var/tmp/quran-rtc/streams/{session_id}/meta.json`

- **POST /stream/chunk**: Append audio chunks to current ayah buffer
  - Params: session_id, file (audio chunk), ayah_end (boolean)
  - When `ayah_end=true`: evaluates accumulated audio, returns full evaluation block, advances to next ayah
  - Evaluation includes `learn` object with translation, tafsir, vocabulary
  - Audio buffer: `/var/tmp/quran-rtc/streams/{session_id}/cur.wav`

- **POST /stream/end**: Finalize session and return summary
  - Returns total blocks_count

#### History & Profile
- **GET /surah_history**: Aggregated historical attempts per ayah
  - Params: user_id, surah, days (default: 30)
  - Returns ayah_history with timestamp and WER for each attempt
  - Sourced from `/var/log/quran-rtc/attempts-*.jsonl`

- **GET /profile**: Retrieve user preferences
  - Params: user_id
  - Returns settings object (silence_threshold, min_pause_ms, voice, etc.)

- **POST /profile**: Store user preferences
  - Params: user_id, settings (JSON string)
  - Stored at `/var/log/quran-rtc/profiles/{user_id}.json`
  - Supports arbitrary key-value pairs

#### Learning Layer
- **GET /learn/ayah**: Educational content for specific ayah
  - Returns: translation, tafsir_short, word_by_word morphology, vocab_highlights, spoken_snippet_en
  - Data sourced from `/opt/quran-rtc/knowledge/` JSON files:
    - `translations_en.json` - English translations
    - `tafsir_en_short.json` - Concise tafsir/commentary
    - `morphology_en.json` - Word-by-word morphology (text, lemma, root, POS, gloss)
  - Seed data includes Surah 1 (Al-Fatiha)
  - Fallback: generates word_by_word from Arabic text if morphology unavailable
  - Automatically attached to `/stream/chunk` responses when `ayah_end=true`

#### Conversational Tutor
- **GET /themes/surah**: Thematic overview and structural outline
  - Data sourced from `/opt/quran-rtc/knowledge/themes_en.json`
  - Seed data: Surah 1 with theme "Orientation to Allah, Mercy, and Guidance"

- **GET /lexicon/lookup**: Morphological word lookup
  - Params: text (Arabic word), lang
  - Searches morphology data for matches
  - Returns: ayah, text, gloss, lemma, root, POS
  - Currently limited to Surah 1, Ayah 1 (expandable)

- **GET /search/term**: Concordance search
  - Params: q (query), lang ("ar" for Arabic text, "en" for English glosses)
  - Arabic: normalized text matching across all verses
  - English: searches in morphology glosses
  - Returns max 200 hits

#### Notes (Learning List)
- **POST /notes/add**: Add vocabulary/grammar/theme/topic to user's learning list
  - Content-Type: application/json
  - Fields: user_id, kind ("vocab"|"grammar"|"theme"|"topic"), arabic, root, gloss, notes, meta
  - Generates unique ID and timestamp
  - Stored in `/var/log/quran-rtc/notes/{user_id}.jsonl`

- **GET /notes/list**: Retrieve learning list
  - Params: user_id, limit (default: 200)
  - Sorted by most recent first

- **POST /notes/remove**: Remove specific note by ID

- **POST /notes/clear**: Clear all notes for user

#### Quiz System
- **GET /quiz/generate**: Generate multiple-choice quiz question
  - Params: surah, ayah, type ("vocab"|"comp")
  - Vocab questions: pick word from morphology, generate distractors
  - Returns: question_id, question text, 4 shuffled choices
  - Correct answer stored at `/var/log/quran-rtc/quiz/{question_id}.json`

- **POST /quiz/grade**: Grade user's answer
  - Params: question_id, answer
  - Returns: ok, correct (boolean)

#### Updated Persona with Command Protocol
- **GET /persona**: Enhanced AI coach instructions with CMD: protocol
  - AI emits structured JSON commands alongside natural language responses
  - Command format: `CMD:{"action":"...", "payload":{...}}`
  - Supported actions:
    - `notes.add` - Add to learning list
    - `notes.remove` - Remove from list
    - `notes.review` - Review notes
    - `lexicon.lookup` - Look up word
    - `themes.get` - Get surah theme
    - `search.term` - Search concordance
  - Enables frontend to parse AI responses and execute actions programmatically

### Changed
- **Backend** `/opt/quran-rtc/backend/server.py` (~1507 lines):
  - Added imports: `uuid`, `random`, `glob`, `time` (consolidated)
  - Added knowledge loading:
    - `KNOWLEDGE_DIR = "/opt/quran-rtc/knowledge"`
    - `_load_json()` helper function
    - `TRANSL_EN`, `TAFSIR_EN`, `MORPH_EN`, `THEMES_EN` data loaders
  - Added `_learn_ayah_payload()` helper for educational content
  - Added `_eval_file_for_ayah()` helper for streaming evaluation
  - Added `STREAM_ROOT = "/var/tmp/quran-rtc/streams"`
  - Added `_stream_paths()` helper for session management
  - Added `NOTES_ROOT = "/var/log/quran-rtc/notes"`
  - Added `_notes_path()`, `_write_note()`, `_read_notes()` helpers
  - Added `NoteIn` pydantic model for note validation
  - Added `_lex_norm_ar()` helper for lexicon searches
  - Updated `/persona` endpoint with conversational tutor instructions and CMD: protocol

- **Apache Configuration**:
  - Added ProxyPass rules for all new endpoints:
    - `/evaluate_surah`, `/stream/start`, `/stream/chunk`, `/stream/end`
    - `/surah_history`, `/profile`, `/learn/ayah`
    - `/themes/surah`, `/lexicon/lookup`, `/search/term`
    - `/notes/add`, `/notes/list`, `/notes/remove`, `/notes/clear`
    - `/quiz/generate`, `/quiz/grade`

- **Knowledge Data** `/opt/quran-rtc/knowledge/`:
  - Created `translations_en.json` with Surah 1 translations
  - Created `tafsir_en_short.json` with Surah 1 tafsir
  - Created `morphology_en.json` with Surah 1, Ayah 1 word-by-word data
  - Created `themes_en.json` with Surah 1 thematic overview
  - Permissions: 755 (dir), 640 (files), owner: quranrtc:quranrtc

- **Storage Directories**:
  - Created `/var/tmp/quran-rtc/streams/` for streaming sessions (750, quranrtc:quranrtc)
  - Created `/var/log/quran-rtc/notes/` for learning lists (750, quranrtc:quranrtc)
  - Created `/var/log/quran-rtc/quiz/` for quiz questions (750, quranrtc:quranrtc)
  - Created `/var/log/quran-rtc/profiles/` for user profiles (750, quranrtc:quranrtc)

### Technical Details

#### Streaming Session Architecture
1. Client calls `/stream/start` → receives session_id
2. Client streams audio chunks via `/stream/chunk`
3. When ayah complete, client sends final chunk with `ayah_end=true`
4. Server evaluates accumulated audio, returns full block with `learn` object
5. Client calls `/stream/end` when surah complete

#### Learning Layer Integration
- `/stream/chunk` automatically attaches `learn` object when `ayah_end=true`
- Includes translation, tafsir, morphology, vocabulary highlights, spoken snippet
- Enables real-time educational feedback during recitation

#### Command Protocol for AI Actions
- Frontend parses AI responses for lines starting with "CMD:"
- Extracts JSON payload and executes corresponding API call
- Example flow:
  1. User: "Add الرحمن to my vocabulary list"
  2. AI speaks: "تم! سأضيف الرحمن إلى القائمة."
  3. AI emits: `CMD:{"action":"notes.add","payload":{"kind":"vocab","arabic":"الرحمن","gloss":"The Entirely Merciful"}}`
  4. Frontend parses CMD, calls `/notes/add`
  5. Frontend shows visual confirmation

#### Dual-Model STT in All New Endpoints
- Primary: gpt-4o-mini-transcribe (fast)
- Secondary: whisper-1 (robust fallback)
- Automatic fallback on HTTP errors
- WER-based retry in `/evaluate_audio` (threshold: 0.35)
- All responses include `stt_model_used` field

### Documentation
- Added comprehensive documentation for 18 new endpoints to `API_DOCUMENTATION.md`
- Organized into new sections:
  - Full Surah Evaluation
  - Streaming API (Ayah-by-Ayah)
  - History & Profile
  - Learning Layer
...
```bash
# Full surah evaluation
curl -X POST https://quran.asimo.io/evaluate_surah \
  -F "surah=1" -F "file=@surah1.mp3" | jq '.blocks | length'

# Streaming session
SID=$(curl -sS -X POST https://quran.asimo.io/stream/start -F "surah=1" | jq -r '.session_id')
curl -X POST https://quran.asimo.io/stream/chunk \
  -F "session_id=$SID" -F "file=@ayah1.wav" -F "ayah_end=true" | jq '.block.learn.translation'

# History
curl -sS "https://quran.asimo.io/surah_history?surah=1&days=7" | jq '.ayah_history'

# Profile
curl -X POST https://quran.asimo.io/profile \
  -F "user_id=test" -F 'settings={"voice":"marin","threshold":0.3}'

# Learning content
curl -sS "https://quran.asimo.io/learn/ayah?surah=1&ayah=1" | jq '.translation'

# Themes
curl -sS "https://quran.asimo.io/themes/surah?surah=1" | jq '.theme'

# Lexicon
curl -sS -G "https://quran.asimo.io/lexicon/lookup" \
  --data-urlencode "text=ٱللَّهِ" | jq '.results[0].gloss'

# Search
curl -sS -G "https://quran.asimo.io/search/term" \
  --data-urlencode "q=Merciful" --data-urlencode "lang=en" | jq '.hits | length'

# Notes
curl -X POST https://quran.asimo.io/notes/add \
  -H "Content-Type: application/json" \
  -d '{"user_id":"test","kind":"vocab","arabic":"الرحمن","gloss":"The Merciful"}'
curl -sS "https://quran.asimo.io/notes/list?user_id=test" | jq '.count'

# Quiz
QID=$(curl -sS "https://quran.asimo.io/quiz/generate?surah=1&ayah=1&type=vocab" | jq -r '.question_id')
curl -X POST https://quran.asimo.io/quiz/grade -F "question_id=$QID" -F "answer=Allah" | jq '.correct'
```

### Future Enhancements
- Expand knowledge data to cover more surahs (currently Surah 1 only)
- Add morphology for all 114 surahs
- Implement comprehension quiz questions ("comp" type)
- Add more command actions for persona (progress, statistics, help)
- Consider caching for frequently accessed learning content
- Add user-specific learning recommendations based on history
